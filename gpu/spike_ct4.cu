...
__global__ void __launch_bounds__(THREADS) ct_step4(Ptrs q, Ptrs o){
    ...
    for(int c=tid;c<FXYZ;c+=THREADS){
        ...
        for(int d=0;d<3;d++){
            ...
            FL[(0*3+d)*FXYZ+c]=(TILET)m1; FL[(1*3+d)*FXYZ+c]=(TILET)m2;
        }
    }
    __syncthreads();
    int li=tx+NG,lj=ty+NG,lk=tz+NG; size_t g=gidx(x0+tx,y0+ty,z0+tz);
    // hydro update from stored fluxes (f32 accumulate)
    F8 fxl=ffl(S,0,li,lj,lk),fxh=ffl(S,0,li+1,lj,lk),fyl=ffl(S,1,li,lj,lk),fyh=ffl(S,1,li,lj+1,lk),fzl=ffl(S,2,li,lj,lk),fzh=ffl(S,2,li,lj,lk+1);
    ...
    #define EZ(a,b,cc) (0.25f*( FF(FL,0,1,a,b,cc)+FF(FL,0,1,(a)-1,b,cc) - FF(FL,0,0,a,b,cc)-FF(FL,0,0,a,(b)-1,cc) ))
    #define EX(a,b,cc) (0.25f*( FF(FL,1,2,a,b,cc)+FF(FL,1,2,a,(b)-1,cc) - FF(FL,1,1,a,b,cc)-FF(FL,1,1,a,b,(cc)-1) ))
    #define EY(a,b,cc) (0.25f*( FF(FL,1,0,a,b,cc)+FF(FL,1,0,a,b,(cc)-1) - FF(FL,0,2,a,b,cc)-FF(FL,0,2,(a)-1,b,cc) ))
...
double ct_run(float* const* qv,float* const* ov,float* const* cc,int nsteps){
    ...
    size_t shmem=sizeof(TILET)*(11*TX*TY*TZ + 6*FXYZ);
    cudaFuncSetAttribute(ct_step4,cudaFuncAttributeMaxDynamicSharedMemorySize,shmem);
    cudaEvent_t t0,t1;cudaEventCreate(&t0);cudaEventCreate(&t1);cudaEventRecord(t0);
    for(int s=0;s<nsteps;s++){ bcc_kernel<<<(n+255)/256,256>>>(Q); ct_step4<<<grid,THREADS,shmem>>>(Q,O); Ptrs t=Q;Q=O;O=t; }
...
int ct_regs(){ cudaFuncAttributes fa;cudaFuncGetAttributes(&fa,ct_step4);return fa.numRegs; }
int ct_shmem(){ return (int)(sizeof(TILET)*(11*TX*TY*TZ+6*FXYZ)); }
}
#endif
#ifndef AS_LIB

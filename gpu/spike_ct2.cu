...
__device__ __forceinline__ F8 ffl(TILET*S,int d,int lx,int ly,int lz){
...
__global__ void __launch_bounds__(THREADS) ct_step2(Ptrs q, Ptrs o){
    extern __shared__ TILET smem[];
    TILET* S=smem;                       // prim+faceB tile: 11 * TX*TY*TZ
    TILET* FL=smem + 11*TX*TY*TZ;        // flux tile: 21 * FXYZ
...
double ct_run(float* const* qv,float* const* ov,float* const* cc,int nsteps){
    ...
    size_t shmem=sizeof(TILET)*(11*TX*TY*TZ + 21*FXYZ);
    cudaFuncSetAttribute(ct_step2,cudaFuncAttributeMaxDynamicSharedMemorySize,shmem);
    cudaEvent_t t0,t1;cudaEventCreate(&t0);cudaEventCreate(&t1);cudaEventRecord(t0);
    for(int s=0;s<nsteps;s++){ bcc_kernel<<<(n+255)/256,256>>>(Q); ct_step2<<<grid,THREADS,shmem>>>(Q,O); Ptrs t=Q;Q=O;O=t; }
...
int ct_regs(){ cudaFuncAttributes fa;cudaFuncGetAttributes(&fa,ct_step2);return fa.numRegs; }
int ct_shmem(){ return (int)(sizeof(TILET)*(11*TX*TY*TZ+21*FXYZ)); }
}
#endif
#ifndef AS_LIB

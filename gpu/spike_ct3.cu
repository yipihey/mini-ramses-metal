...
// ===== STAGED CT: phase A computes each face flux ONCE into a shared flux tile (f16);
// phase B reads it for hydro update (f32 accum) + EMF + face-B. No recompute. =====
#define PTV(S,v,lx,ly,lz) ((float)(S)[(v)*TX*TY*TZ + (lx)+TX*((ly)+TY*(lz))])
#define FX (OX+2)
#define FY (OY+2)
#define FZ (OZ+2)
#define FXYZ (FX*FY*FZ)
// flux store: 7 comps (r,mx,my,mz,E,m1,m2) x 3 dirs; tile cell lx -> fidx = lx-(NG-1)
#define FIDX(lx,ly,lz) (((lx)-(NG-1)) + FX*(((ly)-(NG-1)) + FY*((lz)-(NG-1))))
#define FF(FL,c,d,lx,ly,lz) ((float)(FL)[((c)*3+(d))*FXYZ + FIDX(lx,ly,lz)])

__device__ __forceinline__ P cpt(TILET*S,int lx,int ly,int lz){
    float r=fmaxf(PTV(S,0,lx,ly,lz),SMALLR), ir=1.f/r;
    float mx=PTV(S,1,lx,ly,lz),my=PTV(S,2,lx,ly,lz),mz=PTV(S,3,lx,ly,lz),E=PTV(S,4,lx,ly,lz);
    float bx=PTV(S,5,lx,ly,lz),by=PTV(S,6,lx,ly,lz),bz=PTV(S,7,lx,ly,lz);
    float vx=mx*ir,vy=my*ir,vz=mz*ir;
    float p=fmaxf((GAMMA-1.f)*(E-0.5f*r*(vx*vx+vy*vy+vz*vz)-0.5f*(bx*bx+by*by+bz*bz)),SMALLP);
    return {r,vx,vy,vz,p,bx,by,bz};
}
__device__ __forceinline__ F8 ffl(TILET*S,Ptrs q,int d,int lx,int ly,int lz){
    ...
    size_t gb=gidx(wrap(blockIdx.x*OX-NG+lx,NX),wrap(blockIdx.y*OY-NG+ly,NY),wrap(blockIdx.z*OZ-NG+lz,NZ));
...
__global__ void __launch_bounds__(THREADS) ct_step3(Ptrs q, Ptrs o){
    extern __shared__ TILET smem[];
    TILET* S=smem;                       // prim+faceB tile: 11 * TX*TY*TZ
    TILET* FL=smem + 8*TX*TY*TZ;        // flux tile: 21 * FXYZ
    ...
    for(int c=tid;c<TX*TY*TZ;c+=THREADS){
        ...
        S[0*TX*TY*TZ+c]=(TILET)q.U[0][g];S[1*TX*TY*TZ+c]=(TILET)q.U[1][g];S[2*TX*TY*TZ+c]=(TILET)q.U[2][g];
        S[3*TX*TY*TZ+c]=(TILET)q.U[3][g];S[4*TX*TY*TZ+c]=(TILET)q.U[4][g];
        S[5*TX*TY*TZ+c]=(TILET)q.cx[g];S[6*TX*TY*TZ+c]=(TILET)q.cy[g];S[7*TX*TY*TZ+c]=(TILET)q.cz[g];
    }
    __syncthreads();
    // phase A: each flux cell (lx=fi+NG-1) computes its 3 lower-face fluxes -> FL
    for(int c=tid;c<FXYZ;c+=THREADS){
        int fi=c%FX,fj=(c/FX)%FY,fk=c/(FX*FY); int lx=fi+NG-1,ly=fj+NG-1,lz=fk+NG-1;
        #pragma unroll
        for(int d=0;d<3;d++){
            F8 f=ffl(S,q,d,lx,ly,lz);
            float m1=d==0?f.by:f.bx, m2=d==2?f.by:f.bz;   // d0:(by,bz) d1:(bx,bz) d2:(bx,by)
            FL[(0*3+d)*FXYZ+c]=(TILET)f.r;  FL[(1*3+d)*FXYZ+c]=(TILET)f.mx; FL[(2*3+d)*FXYZ+c]=(TILET)f.my;
            FL[(3*3+d)*FXYZ+c]=(TILET)f.mz; FL[(4*3+d)*FXYZ+c]=(TILET)f.E;  FL[(5*3+d)*FXYZ+c]=(TILET)m1; FL[(6*3+d)*FXYZ+c]=(TILET)m2;
        }
    }
    __syncthreads();
    int li=tx+NG,lj=ty+NG,lk=tz+NG; size_t g=gidx(x0+tx,y0+ty,z0+tz);
    // hydro update from stored fluxes (f32 accumulate)
    #define HY(c) ( (FF(FL,c,0,li,lj,lk)-FF(FL,c,0,li+1,lj,lk)) + (FF(FL,c,1,li,lj,lk)-FF(FL,c,1,li,lj+1,lk)) + (FF(FL,c,2,li,lj,lk)-FF(FL,c,2,li,lj,lk+1)) )
    // f32 update base from GLOBAL (the f16 tile is reconstruction-input only — the GLM lesson)
    o.U[0][g]=q.U[0][g]+DTDX*HY(0); o.U[1][g]=q.U[1][g]+DTDX*HY(1);
    o.U[2][g]=q.U[2][g]+DTDX*HY(2); o.U[3][g]=q.U[3][g]+DTDX*HY(3); o.U[4][g]=q.U[4][g]+DTDX*HY(4);
    // edge EMFs from stored mag fluxes (comp 5=m1, 6=m2)
    #define EZ(a,b,cc) (0.25f*( FF(FL,5,1,a,b,cc)+FF(FL,5,1,(a)-1,b,cc) - FF(FL,5,0,a,b,cc)-FF(FL,5,0,a,(b)-1,cc) ))
    #define EX(a,b,cc) (0.25f*( FF(FL,6,2,a,b,cc)+FF(FL,6,2,a,(b)-1,cc) - FF(FL,6,1,a,b,cc)-FF(FL,6,1,a,b,(cc)-1) ))
    #define EY(a,b,cc) (0.25f*( FF(FL,6,0,a,b,cc)+FF(FL,6,0,a,b,(cc)-1) - FF(FL,5,2,a,b,cc)-FF(FL,5,2,(a)-1,b,cc) ))
    // f32 face-B base from GLOBAL: the curl added to the EXACT old field preserves div·B
    // (f16-rounding the base would break div·B=0); the EMF f16 is consistent across faces -> telescopes.
    o.bx[g]=q.bx[g] - DTDX*((EZ(li,lj+1,lk)-EZ(li,lj,lk)) - (EY(li,lj,lk+1)-EY(li,lj,lk)));
    o.by[g]=q.by[g] - DTDX*((EX(li,lj,lk+1)-EX(li,lj,lk)) - (EZ(li+1,lj,lk)-EZ(li,lj,lk)));
    o.bz[g]=q.bz[g] - DTDX*((EY(li+1,lj,lk)-EY(li,lj,lk)) - (EX(li,lj+1,lk)-EX(li,lj,lk)));
}
#ifdef AS_LIB
extern "C" {
...
double ct_run(float* const* qv,float* const* ov,float* const* cc,int nsteps){
    ...
    size_t shmem=sizeof(TILET)*(8*TX*TY*TZ + 21*FXYZ);
    cudaFuncSetAttribute(ct_step3,cudaFuncAttributeMaxDynamicSharedMemorySize,shmem);
    cudaEvent_t t0,t1;cudaEventCreate(&t0);cudaEventCreate(&t1);cudaEventRecord(t0);
    for(int s=0;s<nsteps;s++){ bcc_kernel<<<(n+255)/256,256>>>(Q); ct_step3<<<grid,THREADS,shmem>>>(Q,O); Ptrs t=Q;Q=O;O=t; }
...
int ct_regs(){ cudaFuncAttributes fa;cudaFuncGetAttributes(&fa,ct_step3);return fa.numRegs; }
int ct_shmem(){ return (int)(sizeof(TILET)*(8*TX*TY*TZ+21*FXYZ)); }
}
#endif
#ifndef AS_LIB
int main(){ printf("spike_ct2: build -DAS_LIB\n"); return 0; }
#endif
